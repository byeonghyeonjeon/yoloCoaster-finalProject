	...
	if ('${keyword}' != '') {
		//페이징처리(검색후)
 		$(".page-link").click(function(){
 			...
 			var keyword = '${keyword}';
			//이동할 페이지 입력
			$("#pageForm").attr("action","${pageContext.request.contextPath}/adminBoard/adminQna?keyword="+keyword);
			$("#pageNo").val(tempNum);
			$("#pageForm").submit();
 		});
	} else {
		//페이징처리
 		$(".page-link").click(function(){
 			var tempNum = $(this).attr("type")
			//이동할 페이지 입력
 			$("#pageForm").attr("action","${pageContext.request.contextPath}/adminBoard/adminQna");
			$("#pageNo").val(tempNum);
			$("#pageForm").submit();
 		});
	}
			...
			$('.btn-anser').on('click', function () {
				var pboard_seq =  $(this).parent().parent().attr("title");
				location.href="${pageContext.request.contextPath}/adminBoard/adminQnaInsert?pboard_seq="+pboard_seq;
			});
			$('.a').on('click', function () {
				var pboard_seq =  $(this).parent().parent().attr("title");
				location.href="${pageContext.request.contextPath}/adminBoard/adminQnaInsert?pboard_seq="+pboard_seq;
			...
			$(".btn-del").click(function(){
				var link = $(this).parent().parent().attr("title");
...
<div class="col-sm-4 col-sm-offset-8">
		<div class="input-group">
			...
			<form id="searchForm"  class="navbar-form" role="search" action="${pageContext.request.contextPath}/adminBoard/adminQna"	method="get">
